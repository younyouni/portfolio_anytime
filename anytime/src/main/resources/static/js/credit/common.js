/* global $, _, _apiServerUrl, jQuery, _diffTime */
...
_gfn = _.extend(_gfn, {
	...
	logPageView: function () {
		...
		var pageLocation = location.origin + location.pathname + location.search;

		if (window.gtag !== undefined) {
			var pageTitle = window._screenName || location.pathname;
...
$().ready(function () {
	...
	$('#submenu').on('click', 'a.more', function () {
		$(this).hide();
		$(this).parents('div.group').nextUntil('div.divider', 'div.group.hidden').css('display', 'inline-block');
	});
	...
	if ($('#container > aside > div.menu').height() === 40) {
		...
		var scrollToActiveMenu = function scrollToActiveMenu() {
