...
$().ready(function () {
	...
	var $title, $containerTitle, $articles, $bubbles;
	var _component = {
		Gallery: (function () {
			var _props = {
				data: []
	...
	var _fn = {
		initiate: function () {
			...
			$bubbles = $container.find('div.bubbles');
			...
			$bubbles.on('click', 'a[href]', function (event) {
			...
			$articles.on('click', '> article > a.article > div.buttons > span.removescrap', function () {
				var $article = $(this).parents('article');
			...
			$articles.on('click', '> article > a.article > ul.status > li.removenotice', function () {
				var $article = $(this).parents('article');
				_fn.removeNoticeArticle($article);
				return false;
			});
		...
		loadContent: function (params) {
			if (params.v) {
				...
				if (_set.searchType) {
					window._screenName = '게시판 - 글 검색';
				} else {
					window._screenName = '게시판 - 글 목록 (게시판)';
				}
		...
		createBestarticleSeasons: function () {
			window._screenName = '게시판 - 글 목록 (BEST 게시판)';
		...
		loadArticles: function () {
			...
			if (Number(_set.boardId)) {
				if (!_set.categoryId) {
					_fn.ajaxNoticeArticles(function (data) {
						_fn.createBubbles(data);
					});
				} else {
					$bubbles.empty();
					$bubbles.addClass('none');
				}
			}
		},
		...
		ajaxNoticeArticles: function (callback) {
			var conditions = {
				mode: "board",
				boardId: Number(_set.boardId),
				isBoardInfoRequired: true
			};
			$.ajax({
				url: _apiServerUrl + '/v2/find/board/article/list',
				xhrFields: {withCredentials: true},
				type: 'POST',
				dataType: 'json',
		...
		createArticles: function (data, isListItem) {
			...
			if (_set.isWritable || _set.authToWrite) {
				var writeArticleButton = $('<a></a>').attr('id', 'writeArticleButton');
				writeArticleButton.text('새 글을 작성해주세요!').appendTo($articles);
			}
			$(data).find('article').each(function () {
				...
				if (isListItem) {
					...
					var articleUrl;
					var $boardName = $('<a></a>').addClass('boardname').html($this.attr('board_name'));
					var $text = $('<p></p>').html($this.attr('text'));
					var $vote = (Number($this.attr('posvote')) > 0) && $('<li></li>').attr('title', '공감').addClass('vote').text($this.attr('posvote')).appendTo($status);
					var $comment = (Number($this.attr('comment')) > 0) && $('<li></li>').attr('title', '댓글').addClass('comment').text($this.attr('comment')).appendTo($status);
					var $time = $('<time></time>').text(_gfn.formatRelativeDate($this.attr('created_at'), true));
					var $attachthumbnail = $('<div></div>').addClass('attachthumbnail');
					...
					if ($this.attr('board_id')) {
						...
						$boardName.attr('href', boardUrl).appendTo($desc);
					}
					if ($this.attr('title')) {
						$title.addClass('medium bold').appendTo($desc);
					}
					$text.addClass('medium').appendTo($desc);
					if ($vote || $comment) {
						$status.appendTo($info);
					}
					$info.appendTo($desc);
					$desc.appendTo($a);
					if ($attachesData.length > 0) {
						...
						if (Number(_set.boardId)) {
							...
							$attachthumbnail.css('background-image', 'url("' + filepath + '")').appendTo($a);
						}
					}
					$time.addClass('small').appendTo($info);
					if ($this.attr('user_nickname') !== '익명' && $this.attr('user_type') !== "disabled") {
						$nickname.addClass('bold');
					}
					$nickname.addClass('small').appendTo($info);
					$('<hr>').appendTo($desc);
				} else {
					...
					// eslint-disable-next-line no-redeclare
					var $time = $('<time></time>').text(_gfn.formatRelativeDate($this.attr('created_at'), false));
					var $status2 = $('<ul></ul>').addClass('status');
					// eslint-disable-next-line no-redeclare
					var $vote = $('<li></li>').attr('title', '공감').addClass('vote').text($this.attr('posvote'));
					// eslint-disable-next-line no-redeclare
					var $comment = $('<li></li>').attr('title', '댓글').addClass('comment').text($this.attr('comment'));
					var $scrap = $('<li></li>').attr('title', '스크랩').addClass('scrap').text($this.attr('scrap_count'));
					...
					$status2.appendTo($a);
					$vote.appendTo($status);
					$comment.appendTo($status);
					$status.addClass('left');
					if (_set.isUser) {
						if (_set.isManageable) {
							// eslint-disable-next-line no-redeclare
							var isNotice = $this.attr('is_notice') === 'true' ? true : false;
							if (!isNotice) {
								$('<li></li>').text('공지로 설정').addClass('setnotice').appendTo($status2);
							} else {
								$('<li></li>').text('공지 내리기').addClass('removenotice').appendTo($status2);
							}
						}
						if (isMine) {
							if (_set.type === 2) {
					...
					$scrap.appendTo($status);
					$status.appendTo($a);
					$('<hr>').appendTo($a);
					$('<span></span>').addClass('posvote').text('공감').appendTo($buttons);
					if ($this.attr('is_scrapped') === '1') {
						$('<span></span>').addClass('removescrap').text('스크랩 취소').appendTo($buttons);
					} else {
		...
		createBubbles: function (data) {
			$bubbles.empty();
			var noticeArticles = data.boardInfo.noticeArticles;
			var popularArticle = data.boardInfo.popularArticle;
			var questionArticles = data.boardInfo.questionArticles;

			if (noticeArticles.length > 0 || questionArticles.length > 0 || popularArticle) {
				$bubbles.removeClass('none');
			}

			var addBubbleGroup = function (articles, category) {
				var $bubble = $('<div></div>').addClass('bubble ' + category);
				$bubble.appendTo($bubbles);
				var $groupIcon = $('<div></div>').addClass('group');
				$groupIcon.appendTo($bubble);
				if (category !== 'popular') {
					var $items = $('<div></div>').addClass('items');
					$items.appendTo($bubble);
				}
				articles.map(function (article) {
					var $item = $('<div></div>').addClass('item');
					var $a = $('<a></a>').attr('href', _fn.encodeUrl({articleId: article.id}));
					var $text = $('<span></span>').html(article.title);
					$text.appendTo($a);
					$a.appendTo($item);
					if (category !== 'popular') {
						$item.appendTo($items);
					} else {
						$item.appendTo($bubble);
					}
				});
			};

			if (noticeArticles.length > 0) {
				addBubbleGroup(noticeArticles, 'notice');
			}
			if (questionArticles.length > 0) {
				addBubbleGroup(questionArticles, 'question');
			}
			if (popularArticle) {
				addBubbleGroup([popularArticle], 'popular');
				var $a = $bubbles.find('> div.popular > div.item > a');
				var $posvote = $('<div></div>').addClass('posvote');
				$posvote.text(popularArticle.posvote).appendTo($a);
			}
		},
		...
		scrapArticle: function ($article) {
			...
			var $scrapbutton = $article.find('div.buttons > span.scrap');
			...
			$.ajax({
				...
				success: function (data) {
					...
					if (response === 0) {
						...
						$scrapbutton.addClass('removescrap').text('스크랩 취소');
					}
				}
			});
		},
		removeScrap: function ($article) {
			var $scrap = $article.find('ul.status > li.scrap');
			var $scrapbutton = $article.find('div.buttons > span.removescrap');
			...
			$.ajax({
				...
				success: function (data) {
					...
					if (response === -1) {
						...
						$scrap.text($scrap.text() - 1);
						$scrapbutton.removeClass('removescrap');
						$scrapbutton.addClass('scrap').text('스크랩');
					}
				}
			});
		},
		setNoticeArticle: function ($article) {
			var $notice = $article.find('li.setnotice');
			if (!confirm('이 글을 공지로 설정하시겠습니까?\n5개를 초과하면, 가장 오래된 공지가 자동으로 내려갑니다.')) {
				return false;
			}
			$.ajax({
				url: _apiServerUrl + '/v2/save/board/notice',
				xhrFields: {withCredentials: true},
				type: 'POST',
				dataType: 'json',
				contentType: 'application/json',
				data: JSON.stringify({articleId: Number($article.data('id'))}),
				success: function (data) {
					if (data.status === 'success') {
						alert('공지로 설정하였습니다.');
						$notice.removeClass('setnotice');
						$notice.text('공지 내리기').addClass('removenotice');
					} else {
						alert('설정에 실패하였습니다.');
					}
				}
		...
		removeNoticeArticle: function ($article) {
			var $notice = $article.find('li.removenotice');
			if (!confirm('공지를 내리겠습니까?')) {
				return false;
			}
			$.ajax({
				url: _apiServerUrl + '/v2/remove/board/notice',
				xhrFields: {withCredentials: true},
				type: 'POST',
				dataType: 'json',
				contentType: 'application/json',
				...
				success: function (data) {
					if (data.status === 'success') {
						$notice.removeClass('removenotice');
						$notice.text('공지로 설정').addClass('setnotice');
					}
				}
			});
		},
